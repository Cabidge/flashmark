...
fn expression_arithmetic() {
    test_render_slide(&mut rhai::Scope::new(), "1 + 2 = @(1 + 2)", "1 + 2 = 3");
}
...
fn expression_variables() {
    ...
    test_render_slide(&mut scope, "Hello, @(name)!", "Hello, World!");
}
...
fn expression_variables_and_arithmetic() {
    ...
    test_render_slide(&mut scope, "@(x) + @(y) = @(x + y)", "1 + 2 = 3");
}

#[test]
fn if_true_and_false() {
    test_render_slide(&mut rhai::Scope::new(), "@if true { true }", " true ");
    test_render_slide(&mut rhai::Scope::new(), "@if false { true }", "");
}
...
fn if_expression() {
    ...
    test_render_slide(&mut scope, "@if x == 1 { 1 } @else { 2 }", " 1 ");
    test_render_slide(&mut scope, "@if x == 2 { 1 } @else { 2 }", " 2 ");
}
...
fn for_expression() {
    ...
    test_render_slide(&mut scope, "@for x in arr { @(x) }", " 1  2  3 ");
}
